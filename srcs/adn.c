...
/*   Updated: 2016/03/04 20:14:10 by amoinier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	color_code(t_env *init, double x, double y)
{
	...
	init->point[(int)y][(int)x + 1]->z != 1 && x - (int)x > 0.995)
	...
	init->point[(int)y + 1][(int)x]->z != 1 && y - (int)y > 0.995)
...
double			adn(t_env *init, double x1, double y1, double xy2[2])
{
	...
	color_code(init, x, y);
	init->dist->distval = sqrt(pow(fabs(x - x1), 2) + pow(fabs(y - y1), 2));
